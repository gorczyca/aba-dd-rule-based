...
object PF1Move extends Move {
  override def isPossible(implicit framework: Framework, dState: DisputeState): Seq[PotentialMove] = {
    ...
    val unexpandedPStatementsLit = framework.unexpandedPStatements.collect { case litArg: LiteralArgument => litArg }.map(_.lit)
    framework.remainingNonBlockedPRules
      .filter( rule => (!dState.pLitArgs.map(_.lit).contains(rule.head) || unexpandedPStatementsLit.contains(rule.head) )
        && rule.body.subsetOf(completePiecesPLit))
