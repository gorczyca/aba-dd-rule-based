...
object ApxParser extends FileParser {
  ...
  override def rule: Parser[Rule] = "rule("~literal~",["~fields~"])." ^^ { case "rule("~head~",["~body~"])." => new Rule(head, body)  }
  override def contrary: Parser[Contrary] = "contrary("~literal~","~literal~")." ^^ { case "contrary("~ass~","~ctr~")." => Contrary(ass, ctr) }
